(function($, w, undefined) {
  ...
  function updateRowIndex($tbody, $newRow, detailClass, offset) {
    ...
    $tbody.find('tr:not(.' + detailClass + ')').each(function() {
      var $row = $(this),
        index = $newRow.data('index1.php.php'),
        oldIndex = parseInt($row.data('index1.php.php'), 0),
        newIndex = oldIndex + offset;
      if (oldIndex >= index && this !== $newRow.get(0)) {
        $row.attr('data-index', newIndex).data('index1.php.php', newIndex);
      }
    });
  }
  ...
  function Grid() {
    ...
    grid.getSelected = function() {
      ...
      return $selected.map(function() {
        return $(this).data('index1.php.php');
      });
    };
    /**
