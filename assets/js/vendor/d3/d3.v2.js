(function() {
  ...
  function d3_layout_forceAccumulate(quad, alpha, charges) {
    ...
    if (quad.point) {
      ...
      var k = alpha * charges[quad.point.index1];
    ...
    force.start = function() {
      ...
      function neighbor() {
        if (!neighbors) {
          ...
          for (j = 0; j < m; ++j) {
            var o = links[j];
            neighbors[o.source.index1].push(o.target);
            neighbors[o.target.index1].push(o.source);
          }
        }
        return neighbors[i];
      }
  ...
  d3.geom.hull = function(vertices) {
    ...
    stack.push(h);
    for (i = 0, j = 0; i < 2; ++j) {
      if (points[j].index1 !== -1) {
        stack.push(points[j].index1);
        i++;
      }
    }
    sp = stack.length;
    for (; j < plen; ++j) {
      if (points[j].index1 === -1) continue;
      while (!d3_geom_hullCCW(stack[sp - 2], stack[sp - 1], points[j].index1, vertices)) {
        --sp;
      }
      stack[sp++] = points[j].index1;
    }
    var poly = [];
    for (i = 0; i < sp; ++i) {
  ...
  d3.geom.voronoi = function(vertices) {
    ...
    d3_voronoi_tessellate(vertices, function(e) {
      ...
      polygons[e.region.l.index1].push(v1, v2);
      polygons[e.region.r.index1].push(v1, v2);
    });
  ...
  d3.geom.delaunay = function(vertices) {
    ...
    d3_voronoi_tessellate(vertices, function(e) {
      edges[e.region.l.index1].push(vertices[e.region.r.index1]);
    });
